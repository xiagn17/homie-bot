...
export class LandlordsTextsService {
  ...
  getObjectFormCommentText(): string {
    return (
      ...
      `\n` +
      `${EMOJI_ATTENTION} <b>Не оставляйте тут свои контакты.</b>\n` +
      `Если вы одобрите анкету съемщика, пользователи получат контакт автоматически.`
    );
  }
  ...
  getObjectFormModerationText(number: number): string {
    return (
      ...
      `\n` +
      `Посмотреть свое объявление вы можете через меню (напишите мне /menu).`
    );
  }
  ...
  getModerationDecisionText(isApproved: boolean): string {
    ...
    return (
      ...
      `Теперь сюда будут приходить анкеты всех заинтересовавшихся. Если возникнут вопросы, обращайтесь в поддержку.`
    );
  }
