  ...
  async getFormText(ctx: MyContext): Promise<string> {
    ...
    if (type === TelegramUserType.renter) {
      ...
      return (
        ...
        `\n` +
        `Если найдется пара минут, я буду очень благодарен коротко заполненной <a href='${LANDLORD_FORM_LINK}'>форме на 3 вопроса</a>. Этим вы сделаешь поиск жильцов еще удобнее!\n` +
        `\n` +
